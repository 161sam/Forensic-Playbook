...
if IoCScanner is None:  # pragma: no cover - legacy guard
    pytest.skip(
        "Legacy module suite not available in this build", allow_module_level=True
    )
...
def sample_iocs():
    ...
    return {
        "hashes": {"sha256": ["5d41402abc4b2a76b9719d911017c592"]},  # hash of "hello"
...
class TestFilesystemAnalysisModule:
    ...
    def test_file_enumeration(self, temp_case_dir, sample_filesystem):
        ...
        result = module.run(None, {"target": str(sample_filesystem), "max_depth": "5"})
    ...
    def test_hidden_files_detection(self, temp_case_dir, sample_filesystem):
        ...
        result = module.run(
            None, {"target": str(sample_filesystem), "analyze_hidden": "true"}
        )

        hidden_finding = next(
            (f for f in result.findings if f["type"] == "hidden_files"), None
        )
    ...
    def test_suspicious_files_detection(self, temp_case_dir, sample_filesystem):
        ...
        (sample_filesystem / "home" / "user" / ".secret_backdoor").write_text(
            "backdoor"
        )

        result = module.run(
            None, {"target": str(sample_filesystem), "detect_suspicious": "true"}
        )

        suspicious_finding = next(
            (f for f in result.findings if f["type"] == "suspicious_files"), None
        )
...
class TestIoCScanner:
    ...
    def test_string_scanning(self, temp_case_dir, sample_filesystem, sample_iocs):
        ...
        (sample_filesystem / "suspicious.txt").write_text(
            "Connect to evil.com on port 4444"
        )
        ...
        matches_finding = next(
            (f for f in result.findings if f["type"] == "ioc_matches"), None
        )
...
class TestTimelineModule:
    ...
    def test_timeline_generation(self, temp_case_dir, sample_filesystem):
        ...
        result = module.run(None, {"target": str(sample_filesystem)})

        assert result.status == "success"
        timeline_finding = next(
            (f for f in result.findings if f["type"] == "timeline_generated"), None
        )
...
class TestNetworkAnalysisModule:
    ...
    def test_suspicious_dns_detection(self, temp_case_dir):
        ...
        dns_queries = [
            ...
            {
                "query": "verylongsubdomainthatissuspeciouslylongandhighentropy123456.evil.com",
                "src_ip": "10.0.0.1",
            },
...
class TestReportGenerator:
    ...
    def test_html_report_generation(self, temp_case_dir):
        ...
        result = module.run(None, {"format": "html"})
    ...
    def test_json_report_generation(self, temp_case_dir):
        ...
        result = module.run(None, {"format": "json"})
