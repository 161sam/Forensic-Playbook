...
class NetworkAnalysisModule(AnalysisModule):
    ...
    def _get_pcap_statistics(self, pcap_file: Path) -> Dict:
        ...
        try:
            ...
            stdout, stderr, rc = self._run_command(["capinfos", str(pcap_file)])
    ...
    def _analyze_protocol_distribution(self, pcap_file: Path) -> Dict[str, int]:
        ...
        try:
            ...
            stdout, stderr, rc = self._run_command(
                ["tshark", "-r", str(pcap_file), "-q", "-z", "io,phs"], timeout=180
            )
    ...
    def _detect_suspicious_dns(self, dns_queries: List[Dict]) -> List[Dict]:
        ...
        for query in dns_queries:
            ...
            if len(parts) > 1:
                subdomain = parts[0]
                if len(subdomain) > 10 and not any(
                    vowel in subdomain for vowel in "aeiou"
                ):
                    suspicious_reasons.append("possible_dga")

            if suspicious_reasons:
                suspicious.append({**query, "reasons": suspicious_reasons})
    ...
    def _detect_suspicious_http(self, http_requests: List[Dict]) -> List[Dict]:
        ...
        for req in http_requests:
            ...
            if any(
                keyword in uri
                for keyword in ["cmd.exe", "powershell", "/etc/passwd", "../"]
            ):
            ...
            if req.get("method") == "POST" and any(
                ext in uri for ext in [".jpg", ".png", ".gif"]
            ):
                suspicious_reasons.append("post_to_image")

            if suspicious_reasons:
                suspicious.append({**req, "reasons": suspicious_reasons})
    ...
    def _detect_suspicious_traffic(
        self, connections: List[Dict], protocol_dist: Dict[str, int]
    ) -> List[Dict]:
