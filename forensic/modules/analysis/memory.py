...
class MemoryAnalysisModule(AnalysisModule):
    ...
    def _analyze_processes(self, dump: Path, profile: str, vol_version: str) -> List[Dict]:
        """Analyze processes"""
        findings = []
    ...
    def _parse_process_list(self, output: str, vol_version: str) -> List[Dict]:
        ...
        for line in lines:
            ...
            if len(parts) >= 4:
                try:
                    processes.append({
                        'pid': int(parts[0]) if vol_version == '3' else int(parts[1]),
                        'ppid': int(parts[1]) if vol_version == '3' else int(parts[2]),
                        'name': parts[2] if vol_version == '3' else parts[0],
                        'threads': int(parts[3]) if vol_version == '3' and len(parts) > 3 else 0
                    })
    ...
    def _detect_suspicious_processes(self, processes: List[Dict]) -> List[Dict]:
        ...
        for proc in processes:
            ...
            # Check unusual parent-child relationships
                ...
                suspicious.append({
                    **proc,
                    'reason': 'unusual_parent'
                })
        
        return suspicious
    
    def _analyze_network(self, dump: Path, profile: str, vol_version: str) -> List[Dict]:
    ...
    def _analyze_registry(self, dump: Path, profile: str, vol_version: str) -> List[Dict]:
