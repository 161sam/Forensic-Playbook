...
class QuickTriageModule(TriageModule):
    ...
    def run(self, evidence: Optional[Evidence], params: Dict) -> ModuleResult:
        ...
        findings = []
        errors = []
        metadata = {"target": str(target), "triage_start": timestamp}
    ...
    def _analyze_users(self, target: Path) -> List[Dict]:
        ...
        try:
            ...
            suspicious_users = [
                u for u in users if u["has_interactive_shell"] and int(u["uid"]) >= 1000
            ]
    ...
    def _check_ssh_keys(self, target: Path) -> List[Dict]:
        ...
        try:
            ...
            search_paths = [target / "root" / ".ssh", target / "home"]
            ...
            for search_path in search_paths:
                ...
                for key_file in search_path.rglob("id_*"):
                    if key_file.is_file() and not key_file.name.endswith(".pub"):
                        ssh_keys.append(
                            {"path": str(key_file), "size": key_file.stat().st_size}
                        )
