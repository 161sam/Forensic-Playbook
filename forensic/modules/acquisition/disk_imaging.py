...
class DiskImagingModule(AcquisitionModule):
    ...
    def run(self, evidence: Optional[Evidence], params: Dict) -> ModuleResult:
        ...
        source = Path(params["source"])
        output = Path(
            params.get("output", self.output_dir / f"disk_image_{timestamp}.img")
        )
        ...
        missing = [
            name for name in required.get(tool, []) if not self._verify_tool(name)
        ]
        if missing:
            guidance = (
        ...
        try:
            if tool == "dd":
                success, image_metadata = self._image_with_dd(
                    source, output, block_size
                )
            elif tool == "ddrescue":
    ...
    def _get_device_info(self, device: Path) -> Dict:
        ...
        try:
            ...
            stdout, _, _ = self._run_command(
                ["lsblk", "-ndo", "MODEL,SERIAL", str(device)]
            )
            if stdout.strip():
    ...
    def _image_with_dd(self, source: Path, output: Path, block_size: str) -> tuple:
        ...
        metadata = {"dd_log": str(log_file), "dd_returncode": returncode}
        ...
        if log_file.exists():
            with open(log_file) as f:
                for line in f:
                    if (
                        "non-tried" in line
                        or "non-trimmed" in line
                        or "bad-sector" in line
                    ):
                        bad_sectors += 1
    ...
    def _image_with_ewfacquire(self, source: Path, output: Path) -> tuple:
        ...
        cmd = [
            ...
            "-u",  # Unattended mode
            "-f",
            "encase6",  # EnCase 6 format
            str(source),
        ]

        stdout, stderr, returncode = self._run_command(cmd, timeout=3600)

        metadata = {"ewfacquire_returncode": returncode, "format": "encase6"}
