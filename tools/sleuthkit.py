...
class SleuthkitWrapper:
    ...
    def get_inode_info(self, inode: int, partition: Optional[int] = None) -> Dict:
        """
    ...
    def read_blocks(
        self, start_block: int, count: int, partition: Optional[int] = None
    ) -> bytes:
        """
    ...
    def find_filename(
        self, inode: int, partition: Optional[int] = None
    ) -> Optional[str]:
        """
    ...
    def _run_command(
        self, cmd: List[str], timeout: int = 300, capture_binary: bool = False
    ) -> Tuple[bytes, str, int]:
        """
        Run TSK command
