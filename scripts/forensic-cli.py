...
@click.option('--config', type=click.Path(exists=True), default=None, help='Config file')
...
def cli(ctx, workspace, config, verbose):
    ...
    framework.register_module('disk_imaging', DiskImagingModule)
    framework.register_module('memory_dump', MemoryDumpModule)
    framework.register_module('filesystem_analysis', FilesystemAnalysisModule)
    framework.register_module('ioc_scan', IoCScanner)
...
@click.option('--type', '-t', 'evidence_type', 
...
def quick_triage(ctx, target_path, name, investigator):
    ...
    case = framework.create_case(
        name=name,
        description="Quick triage investigation",
        investigator=investigator
    )
