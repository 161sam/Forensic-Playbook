            ...
            for ioc in iocs:
                for variant in ioc_variants.get(ioc.value, [ioc.value]):
                    if variant.lower() in s_lower:
                        matches.append(
                            Match(ioc=ioc, file_path=str(file_path), context=s[:200])
                        )
        return matches
...
def scan_npm_lockfiles(scan_path: Path) -> List[Dict[str, str]]:
    ...
    for root, dirs, files in os.walk(scan_path):
        ...
        dirs[:] = [
            d for d in dirs if not should_exclude(Path(root) / d, DEFAULT_EXCLUDES)
        ]
...
def scan_package_lock(file_path: Path) -> List[Dict[str, str]]:
    ...
    def check_deps(deps):
        for name, info in deps.items():
            ...
            if (
                name in MALICIOUS_NPM_PACKAGES
                and version in MALICIOUS_NPM_PACKAGES[name]
            ):
                findings.append(
                    {"package": f"{name}@{version}", "file": str(file_path)}
                )
            # Recurse
    ...
    if "packages" in data:
        for _pkg_path, info in data.get("packages", {}).items():
            ...
            if (
                name
                and name in MALICIOUS_NPM_PACKAGES
                and version in MALICIOUS_NPM_PACKAGES[name]
            ):
                findings.append(
                    {"package": f"{name}@{version}", "file": str(file_path)}
        ...
        for pkg_name, versions in MALICIOUS_NPM_PACKAGES.items():
            for version in versions:
                if f"{pkg_name}@{version}" in line or f"{pkg_name}/{version}" in line:
                    findings.append(
                        {"package": f"{pkg_name}@{version}", "file": str(file_path)}
                    )
                        ...
                        "rules": [
                            {
                                ...
                                "fullDescription": {
                                    "text": "The file contains a known Indicator of Compromise from the provided IoC list."
                                },
        ...
        sarif["runs"][0]["results"].append(
            {
                ...
                "level": "error",
                "message": {
                    "text": f"IoC detected: {match.ioc.type} = {match.ioc.value}"
                },
                "locations": [
                    {
                        "physicalLocation": {
                            "artifactLocation": {"uri": match.file_path},
                            "region": {"startLine": match.line_number or 1},
                        }
                    }
                ],
                "properties": {
...
Examples:
  ...
  python3 ioc_scan.py --path /var/log --ioc-file IoCs.json \\
    ...
    parser.add_argument(
        "-f",
        "--format",
        choices=["json", "csv", "sarif", "text"],
        default="text",
        help="Output format",
    )
    parser.add_argument(
        "-o", "--out-file", type=Path, help="Output file (default: stdout)"
    )
    parser.add_argument(
        "--out-dir", type=Path, help="Output directory for all artifacts"
    )
    parser.add_argument(
        "-t", "--timeline", action="store_true", help="Enable timeline correlation"
    )
    parser.add_argument(
        "-x",
        "--extract-strings",
    ...
    if args.verbose:
        ...
        print(
            f"  Files with matches: {stats.get('files_with_matches', 0)}",
            file=sys.stderr,
        )
