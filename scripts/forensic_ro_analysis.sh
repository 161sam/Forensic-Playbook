...
_umount_all() {
  # in umgekehrter Tiefe aushängen
...
# Mounten aller Partitionen read-only unter ${MNT}/pN
for pn in "${PARTS[@]}"; do
  dev="/dev/${pn}"
  mp="${MNT}/${pn}"
...
# Root-FS heuristisch bestimmen: Kandidat mit /etc/os-release
ROOT_MP=""
for pn in "${PARTS[@]}"; do
  mp="${MNT}/${pn}"
...
if [[ -z "$ROOT_MP" ]]; then
  # Fallback: größte Linux-Partition
  ROOT_CAND="$(lsblk -b -ln -o NAME,SIZE,FSTYPE "$DEVICE" | awk '$3 ~ /ext|xfs|btrfs/ {print $1,$2}' | sort -k2,2nr | head -n1 | awk '{print $1}')"
...
mkdir -p "${OUT}/systemd"
find "${ROOT_MP}/etc/systemd" -maxdepth 3 -type f -name "*.service" -print0 | xargs -0 -I{} bash -lc 'rel="${1#'"${ROOT_MP}"'/}"; echo "$rel";' _ {} > "${OUT}/systemd/etc_services.txt" || true
find "${ROOT_MP}/usr/lib/systemd" -maxdepth 3 -type f -name "*.service" -print0 | xargs -0 -I{} bash -lc 'rel="${1#'"${ROOT_MP}"'/}"; echo "$rel";' _ {} > "${OUT}/systemd/usrlib_services.txt" || true
# Enabled-Links / wants
find "${ROOT_MP}/etc/systemd/system" -type d -name "*.wants" -print -o -type l -print > "${OUT}/systemd/enabled_wants.txt" || true
# Timer
find "${ROOT_MP}" -type f -name "*.timer" -print | sed "s#^${ROOT_MP}/##" > "${OUT}/systemd/timers.txt" || true
...
find "${OUT}/ssh/home_keys" -type f -name "id_*" -exec chmod 000 {} + 2>/dev/null || true # Sichtschutz
...
find "${ROOT_MP}/etc/systemd" -type f \( -name "*.path" -o -name "*.timer" \) -print \
  | sed "s#^${ROOT_MP}/##" > "${OUT}/persistence/systemd_paths_timers.txt" || true
# Bash/Zsh-Autoruns in Homes
find "${ROOT_MP}/home" -maxdepth 2 -type f \( -name ".bashrc" -o -name ".zshrc" -o -name ".profile" -o -name ".bash_profile" \) \
  -print | sed "s#^${ROOT_MP}/##" > "${OUT}/persistence/user_shell_autoruns.txt" || true
...
# SUID/SGID nur Dateilisten + Hashes
...
# rpm-based fallback (falls nicht Debian)
